...
<form action="item_create.php" method="post" enctype="multipart/form-data" onsubmit="return submit_check()">

  <div class="edit_box">
    <h4 style="margin-bottom:12px; margin-top: 40px;">게시판 선택</h4>
    ...
    <input type="text" name="item_title" id="uid" class="form-control" placeholder="상품 이름을 입력하세요." required autofocus><BR>

    <h4 style="margin-top:20px">가격 입력</h4>
    <input type="text" name="item_price" id="item_price" class="form-control" placeholder="가격을 입력하세요." required autofocus><BR>
    ...
    if(!$.isNumeric($('#item_price').val())){
      ...
      check = false;
        ...
        if($('#description').val() === "<p><br></p>" || !description_check){
          ...
          if(img_upload_check){
            swal("상품 등록!", "상품이 등록 되었습니다!", "success");
          }else{
            swal("등록 불가!", "상품 이미지를 등록해주세요", "warning");
            check = false;
          }
        }
      }
