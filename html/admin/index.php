...
if(!isset($_SESSION['user_id'])){
  ...
  $sql2 = "SELECT * FROM user_tb where date(create_date) = date(NOW())";
  ...
  $sql11 = "select sum(item_price) from buy_item where date(buy_date) >= date_format(now(), '%Y-%m-01')";

  $sql12 = "select count(*) as cnt from buy_item where DATE(buy_date) >= DATE_FORMAT(NOW(), '%Y-%m-01')";
  ...
  $sql15 = "select * from buy_item where date(buy_date) = date(now())";
  ...
  $total_sell_item = mysqli_num_rows(mysqli_query($conn,$sql18));

  $best_sell_item_persent = round(($best_item_count['max(mycount)']/$total_sell_item)*100,1);

  $total_visit_user_persent = $total_visit_user_count['max(mycount)'];

  $top_view_item_persent =  round(($top_view_item_count['max(mycount)']/$total_item_view)*100,1);

  $best_client_persent = $best_client_count['max(mycount)'];
				...
				<div class="row">
					...
					<div class="cirStats">
						  	<div class="row">
								...
								<div class="col-xs-12 col-sm-6 col-md-6">
										<div class="card-panel text-center">
											<h4>가장 많이 방문한 고객<br> <p style="text-transform:none"><?=$total_visit_user_name['id']?> 님</p></h4>
											<div class="easypiechart" id="easypiechart-red" data-percent="100" ><span class="percent"><?= $total_visit_user_persent?>회</span>
											</div>
										</div>
								</div>
								<div class="col-xs-12 col-sm-6 col-md-6">
										<div class="card-panel text-center">
											<h4>가장 많이 결제한 고객<br> <p style="text-transform:none"><?=$best_client['id']?> 님</p> </h4>
											<div class="easypiechart" id="easypiechart-teal" data-percent="100" ><span class="percent"><?=$best_client_persent?>회</span>
											</div>
										</div>
								</div>
								<div class="col-xs-12 col-sm-6 col-md-6">
